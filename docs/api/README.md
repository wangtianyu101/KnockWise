...
## 11. V2 智能沉淀层 `/api/v2/*` （2026-07-03）

V2 新增 6 个端点（路径前缀 `/api/v2`，所有端点走 JWT + 响应头加 `X-API-Version: v2.0`）。
完整契约见 `docs/tasks/2026-06-28-new-feature-v2-smart-sediment/api-spec.md`。

### `GET /api/v2/dashboard/summary`

Dashboard 顶部"今日学习总结"卡。Redis 缓存 TTL 1h，LLM 失败降级（`_fallback=true`）。

```json
// Query: ?date=2026-06-28 (可选, 默认 today)
// Response 200
{
  "title": "今日学习总结",
  "date": "2026-06-28",
  "yesterday_count": 8,
  "mastered": [{"topic": "React Hooks", "error_rate": 0.2, ...}],
  "weak_shift": [{"from_topic": "网络层", "to_topic": "状态管理"}],
  "body": "昨天你答了 8 道题，掌握 2 个新 topic...",
  "_fallback": false
}
// Errors: 401 (no JWT), 422 (date format)
```

### `GET /api/v2/profile/weekly?week=2026-W26`
...
// Response 200
{
  ...
  "weak_topics": [...],
  "body": "本周你答了 42 题...",
  "trajectory": {"2026-W15": {"mastered_count": 2}, ..., "2026-W26": {"mastered_count": 9}}
}
// Errors: 401, 422 (week format)
```

### `GET /api/v2/profile/monthly?month=2026-06`

月报 + 落库 `monthly_reports.summary_stats`。
...
// Response 200
{
  ...
  "total_questions": 168,
  "mastered_count": 28,
  "trajectory": {"2026-01": {"mastered_count": 5}, ...},
  "summary_stats": {"narrative": "...", "saved_to_db": true, "monthly_report_id": "uuid"}
}
// Errors: 401, 422 (month format)
```

### `POST /api/v2/profile/refresh`

手动刷新画像（触发 weekly_full_refresh + DEL 3 Redis cache key）。
...
// Errors: 401, 429 (60s 内 1 次)
```

### `GET /api/v2/knowledge/recent-sediments?limit=5`

最近 N 个学习沉淀文件（扫 `~/Obsidian/coding/learning/` + `interview/`）。vault 不存在返空 list（决策 7A）。
...
// Errors: 401, 422 (limit > 20)
```

### `POST /api/v2/obsidian/sync?date=2026-06-28`

手动触发 Obsidian 同步（vault 后创建时用）。vault 不存在返 `synced_count=0`。
...
// Response 200
{
  "date": "2026-06-28",
  "synced_count": 3,
  "files": [{"rel_path": "learning/2026-06-28.md", "success": true, ...}]
}
// Errors: 401, 422 (date format)
```
...
- **认证**：所有 6 个端点必须带 `Authorization: Bearer <token>`
- **错误响应统一格式**：
  ```json
  {"error": {"code": "VALIDATION_ERROR", "message": "...", "details": {...}}}
  ```
- **降级策略**：失败返 200 + 降级版（`_fallback=true`），不返 5xx
- **限流**：60s 内 N 次（spec §3.2 表格，slowapi V2.5 优化阶段接入）
- **缓存**：summary:dashboard:{user_id} / summary:profile:{user_id}（TTL 1h）
