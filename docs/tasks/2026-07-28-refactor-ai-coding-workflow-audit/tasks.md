...
### T3: ✅ 已实现 · 状态 reducer 与失效语义

- [x] T3: 实现 ordered events → projection，并处理 commit/spec 变化后的 STALE/NOT_RUN
...
### T5: ✅ 已实现 · 决定性 projector 与 drift 检测

- [x] T5: 生成 task JSON/YAML/Markdown projection，支持零 diff 重放与 source-hash mismatch
  ...
  - **实际**: 25 min；RED=projector 模块不存在；GREEN=11/11 target、
    158/158 相关回归 passed；固定四文件逐字节决定性输出、JSON/YAML round-trip、
    重放零 diff/mtime 不变、marker 与正文 drift fail-closed；PyYAML 直接 pin 6.0.3；
    test-quality 0 violations；Python 3.9 语法解析通过；commit `860f764`；独立
    verifier PASS（17/17 对抗探针，偏差 0）
...
- [x] T6: 实现 orphan branch bootstrap、只读加载、临时 worktree 和 FORMAT 校验
  ...
  - **实际**: 40 min；RED=git_store 模块不存在；GREEN=10/10 target、
    ...
    violations；Python 3.9 语法解析通过；commit `71b34dd`；独立 verifier
    PASS（9 个真实 Git 对抗探针，偏差 0）

### T7: ✅ 已实现 · 本地原子追加与 CAS

- [x] T7: 使用 `fcntl.flock`、临时 worktree 和 `git update-ref old_oid` 原子提交事件+投影
  - **文件**: `scripts/workflow_state/git_store.py`
  ...
  - **实际**: 40 min；RED=7/7 因 append/CAS 接口不存在失败；GREEN=7/7
    target、168/168 Core+store passed；真实 Git 验证 event+四 projection 同一 commit、
    canonical 历史不改写、旧 projection drift fail-closed、非阻塞 `flock` 冲突
    BLOCKED、unsafe path/错误 hash 不移动 ref、`update-ref new old` CAS 失败不覆盖；
    hook `GIT_*` 污染环境回归通过；test-quality 0 violations；Python 3.9 语法通过；
    commit `dc941d9`；独立 verifier PASS（19/19 真实 Git 探针，偏差 0）

### T8: ✅ 已实现 · 幂等与远端并发重试

- [x] T8: 拒绝重复逻辑事件，处理 fast-forward reject，最多三次重放后返回 BLOCKED
  - **文件**: `scripts/workflow_state/git_store.py`
  ...
  - **实际**: 40 min；RED=6/6 因远端 retry API 不存在失败；GREEN=8/8
    target、176/176 Core+store passed；真实 bare remote 验证同 key 幂等零新 commit、
    同 key 异内容 fail-closed、双 actor 旧 head 重放后两事件均保留、第三次 push
    成功、三次 reject 恢复权威 head 并 BLOCKED、权威不可用不产生本地事件；普通
    fast-forward refspec 且拒绝 option-like remote；test-quality 0 violations；Python
...
- [x] T10: 实现 observe-commit/run-test，验证 Git object，直接执行 argv 并记录 rc/counts/digest
  ...
  - **实际**: 35 min；RED=7/7 因 observe-commit/run-test 命令不存在失败；GREEN=8/8
    target、192/192 Core+CLI passed；真实 subprocess 验证完整 40 字符 SHA 及
    ...
    violations；Python 3.9 AST 解析通过；commit `f4fd4d1`；完整 backend hook
    1061 passed / 2 skipped / 14 xfailed；独立 verifier PASS（15/15 真实 Git
    对抗探针，偏差 0）
...
| T10 | `test_taskctl_observers.py` | 真实 commit/test evidence | REQ-004/005 | SCN-006/007/008 | TC-006/007/008 | L2 | `f4fd4d1` | PASS | PASS | PENDING |
