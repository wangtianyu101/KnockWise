...
- [ ] T1: 实现 TaskEvent/TaskProjection 枚举、字段校验和 schema version Gate
...
- [ ] T4: 将 event type → allowed actor 编码为 fail-closed policy，拒绝跨 Actor 自签
...
|---|---|---|---|---|---|---|---|---|---|---|
| T1 | `test_workflow_state_models.py` | 未知 schema 拒绝 | REQ-010 | SCN-017 | TC-017 | L1 | PENDING | NOT_RUN | NOT_RUN | PENDING |
...
|---|---|---:|---|---|---|---|
| T1～T20 | — | — | NOT_RUN | NOT_RUN | PENDING | 实施后逐项回写 |
