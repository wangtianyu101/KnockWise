...
| 分布式活动 | 证据 | 结果 |
|---|---|---|
| L1 静态检查 | `git diff --check` | ✅ exit 0 |
| L2 单元测试 | 纯 Markdown 指令重构，无可执行业务逻辑 | N/A（无测试人口） |
| L4 review / verifier | 第二轮独立 verifier 逐段对照旧主账 | ✅ PASS |

## L3 整合测试

- **范围**：Codex 入口、Claude 导入入口、公共正文和工具专属正文的组合契约。
- **命令**：`test "$(sed -n '1p' CLAUDE.md)" = '@AGENTS.md'`；`rg` 检查循环引用与工具边界。
- **结果**：PASSED。
- **证据**：导入目标存在；`AGENTS.md` 无 `CLAUDE.md` 反向引用；Claude 专属工具语法只在 `CLAUDE.md`。

## L5 staging 运行时验证

- **环境**：仓库工作树的下一会话加载配置；本任务不涉及应用服务，用户明确要求不启动服务。
- **验证方式**：核对 Claude 原生 `@AGENTS.md` 入口、目标文件存在性、Codex 32 KiB 上限和无循环关系。
- **结果**：PASSED（配置契约级）。
- **残留检查**：Codex / Claude 新会话启动后可分别用“列出当前项目指令源”再次人工确认；当前会话不会热重载入口文件。
