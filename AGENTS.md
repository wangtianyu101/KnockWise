...
> 任何 AI 助手（Codex、Claude Code 等）在本项目工作时必须遵守。
>
...
| **6** | **复盘** | verify 后自动起草，用户确认完成 | 量化偏差、根因、改进与规则沉淀 | `retro.md` + 更新 `AGENTS.md` / `DOD.md` / 模板 |
...
> 1. **implementation 已落入 commit 后**，将该任务标为 `- [x] T<n>` 并记录 commit hash；`[x]` 只表示已实施，可与 test/verifier `FAIL` 共存；禁止写裸 `✅ DONE`
...
| Agent | 角色 | 触发时机 | 实现 |
|---|---|---|---|
| **writer** | 实施代码 / 测试 | 用户说"开始实施"进入步 4 | 使用当前环境实际提供的读写与编辑工具 |
| **verifier** | 独立路径校验 | writer 完成一个 commit 单元 | 使用当前环境实际提供的独立 agent 工具创建全新上下文 · **不复用 writer 上下文** |
...
- ❌ verifier 复用 writer 上下文 — 失去独立路径意义，必须新建独立 verifier 上下文
- ❌ FAIL 后 writer 反复微调不收敛 — 报用户，不要无止境循环
- ❌ 跳过 verifier 直接写下一个 commit — § 6.7 没生效
- ❌ 调研阶段决策后没回写 `research.md` § 三/七/八 — 违反 § 6.8
- ❌ 假设当前环境一定存在某个 Workflow/orchestration tool；必须先以实际可用工具为准
...
- **任何未来 KnockWise 项目**的决策都按本节结构（AGENTS.md 是项目级 hook 准则 · 跨项目生效）
...
- 默认使用当前环境实际暴露的一次性独立 agent 工具创建 verifier；只读 verifier 可以与 writer 并行。
- ≥3 个 verify/fix cycle 时，把每轮 PASS/FAIL、偏差和修复记录写入 `tasks.md`，避免只存在上下文中。
- 当前运行环境若明确暴露确定性 Workflow/orchestration 工具，可以升级使用；**未暴露时不得声称存在或虚构调用方式**。
- 安全、权限、数据迁移等关键改动可开两个独立 verifier 做对抗式检查；结论冲突时交用户拍板。
- 两轮修复仍无法收敛时停止自动循环，列出剩余偏差和证据请求用户决策。

### 6.11 规则退役（2026-07-25 新增 · 流程防膨胀）

> 规则只减不增；新规则按 `feedback-decisions-sync-to-decisions` 模板落 issues.md；旧规则按本节流程退役。

- **删除条件（三选一满足即触发）**：
  1. ≥ 2 个 PR 周期未被引用 / 文档引用
  2. 已有工具 / pre-commit / checker 自动覆盖
  3. 在 `docs/issues.md` 标 "退役" 段
- **退役≠删除**：先标 ❌ 退役（保留 1 个季度）· 季度后归档 `docs/archive/rules-deprecated/`
- **季度归档**：每 3 个月在 `docs/issues.md` 列已退役规则清单
- **retro 必须写明**：替换 / 做错什么 / 替代什么
- **退役清单示例**（首次应用待 § 6.10 实施后或下次 4 类真实任务验证后）：
  - 本节列出当前可退役候选 (待验证)
  - 例如: § 6.4 违反处置 (与 § 6.7 verify-loop 重复) / § 6.5 任务完成自动更新 (与 pre-commit tasks.md 同步重复)
...
**强约束**：开工前**必须**先 `./scripts/start.sh` 把基础设施起起来（不在 AGENTS.md 详述，见 local-dev.md）。
...
### 8.3 git 状态（不在 AGENTS.md 维护）

- 当前 commit / 分支 / 未 push 数量：`git log --oneline -10` · `git status -sb`
- 历史完整列表 `git log --oneline`（不在 AGENTS.md 复制粘贴，会自然 stale）

### 8.4 动态事项入口

`AGENTS.md` 不保存会自然过期的版本、commit 数和待决策列表：遗留问题统一写 [`docs/issues.md`](docs/issues.md)，实施里程碑写 [`docs/rules/milestones.md`](docs/rules/milestones.md)，具体任务进度写对应 `tasks.md`。
