...
def validate_failure_action_length(actions: List[str], layer: str) -> list[str]:
    ...
    if layer in ("L2", "L3"):
        for i, action in enumerate(actions):
            if len(action) < 30:
                violations.append(
                    f"failure_action[{i}]: 字符数 {len(action)} < 30（spec SCN-P1.8.3）"
                )
    return violations
...
def check_metric_dict(file_path: str, enforce_l2: bool = False) -> int:
    ...
    try:
        ...
        print(f"::error::YAML parse error: {file_path}: {e}")
        return 1

    if not isinstance(data, dict):
        print(f"::error::{file_path}: top-level must be mapping, got {type(data).__name__}")
        return 1
    ...
    # L1 最小集：metric_id 正则 + layer 字段
    if layer == "L1":
        # 仅校验 metric_id 正则
        try:
            MetricDict.model_validate({**data, **{"num_event": {"name": "x", "file": "x", "line": 1}}})
    ...
    if violations:
        for v in violations:
            print(f"::error::{v}")
        print(f"\n❌ {len(violations)} check-metric-dict violation(s)")
        return 1
